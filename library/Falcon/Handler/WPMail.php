...
class Falcon_Handler_WPMail implements Falcon_Handler {
	...
	public function send_mail( $users, Falcon_Message $message ) {
		...
		foreach ( $users as $user ) {
			...
			$headers = array(
				'From'         => sprintf( '"%s" <%s>', mb_encode_mimeheader( $author ), mb_encode_mimeheader( $from ) ),
			...
			$result = wp_mail(
				sprintf( '"%s" <%s>', mb_encode_mimeheader( $user->display_name ), mb_encode_mimeheader( $user->user_email ) ),
				$message->get_subject(),
				$message->get_html(),
